...
co2$date <- as.Date(paste(co2$year, co2$month, "1", sep = "-"), format = "%Y-%m-%d") #using as.Date()
head(co2)
...
ggplot(c02_max_months, aes(x = date, y = average)) +
  geom_line() +
  geom_point(color = "forestgreen", size = 0.5) + #makes points "forestgreen"
  labs(x = "Date", y = "Average CO2 (ppm)") +
